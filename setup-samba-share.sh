...
print_usage() {
	...
	output "   docker run bmst/samba-share \"$container\" | sh"
	output ""
	output " OR - depending on your Docker Host's location of its docker binary"
	output ""
	output "   DOCKER=/usr/local/bin/docker /usr/local/bin/docker run bmst/samba-share \"$container\" | /bin/sh"
...
execute_in_sh() {
	...
	volumes=`$DOCKER inspect --format='{{range \$k,\$v := .Config.Volumes}}{{println \$k}}{{end}}' "$container" |  grep -v -E "^$"
             ...
             $DOCKER inspect --format='{{range \$v :=             .Mounts}}{{println \$v.Destination}}{{end}}' "$container" |  grep -v -E "^$"`
